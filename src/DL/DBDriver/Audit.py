...
from .Att import *
from .Enums import MutationMode
...
class Audit( object ):
    ...
    def add_audit_def(self, audit_names, table_def) -> dict:
        ...
        if not audit_names:
            return table_def
        row_def_audit = {key: value for key, value in table_def.items()}
        for i in range( len( audit_names ) ):
            row_def_audit[len( table_def ) + 1 + i] = self._get_att( audit_names[i] )
    ...
    def _get_att(att_name, mode=None, pgm=__name__) -> Att:
        ...
        if att_name == Datetime_creation:
            return Att(
                att_name,
                value=str( datetime.datetime.now().isoformat() if not mode or mode == MutationMode.C else NC ),
                type=AttType.Timestamp )

        elif att_name == User_creation:
            return Att( att_name, getpass.getuser() if not mode or mode == MutationMode.C else NC )

        elif att_name == Program_creation:
            return Att( att_name, pgm if not mode or mode == MutationMode.C else NC )

        elif att_name == Datetime_mutation:
            return Att(
                att_name,
                value=str( datetime.datetime.now().isoformat() if mode == MutationMode.U else 0 ),
                type=AttType.Timestamp )

        elif att_name == User_mutation:
            return Att( att_name, getpass.getuser() if mode == MutationMode.U else EMPTY )

        elif att_name == Program_mutation:
            return Att( att_name, pgm if mode == MutationMode.U else EMPTY )
