...
class DBDriver( object ):
    ...
    def _get_ffd_table_def(self, table_name) -> dict:
        ...
        try:
            ...
            # In case of an specified definition (e.g. imported file) the FFD definition does not need to exist.
    ...
    def _cache_table(self, table_name, table_def=None, audit_type=None):
        ...
        force_cache = False
        if table_def:
            self._table_defs[table_name] = table_def
            force_cache = True
    ...
    def _is_row_changed(self, row, atts):
        return any(
            ...
            != str( row[self._colno_from_cache( att.name )] )
            for att in atts
        )
    ...
    def select(self, table_name, **kwargs) -> list:
        ...
        if names:
            if mode == FetchMode.First:
                row = self.fetch_one( table_name, **kwargs )
                out_rows = [self._get_value_from_row( att_name, row ) for att_name in names]
            else:
                rows = self.fetch( table_name, **kwargs )
                out_rows = [[self._get_value_from_row( att_name, row ) for att_name in names] if names
                            else self._get_value_from_row( name, row ) for row in rows]
        elif name:
            if mode == FetchMode.First:
                row = self.fetch_one( table_name, **kwargs )
                out_rows = [self._get_value_from_row( name, row )]
            else:
                rows = self.fetch( table_name, **kwargs )
                out_rows = [self._get_value_from_row( name, row ) for row in rows]
        else:
            if mode == FetchMode.First:
    ...
    def _get_value_from_row(self, att_name, row):
        """ DB row has an Id."""
        if not row:
            return None
        if att_name == ID:
            ...
            c = self._colno_from_cache( att_name )
            return row[c] if c else None

    def _colno_from_cache(self, att_name) -> int or None:
        """ 1-based """
